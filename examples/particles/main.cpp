...
int main() {
   ...
   //--------------------------------------------------------------------------------------
    ...
    //default settings
    constexpr cyclone::real SPHERE_RADIUS = 0.125 * 0.75f;
    constexpr int MAX_CONTACTS = 256;
    constexpr int RESTITUTION = 1;
    ...
    colliders.push_back(new cyclone::PlaneCollider(cyclone::Vector3(0, 10, 0), cyclone::Vector3(0, -1, 0))); //ceiling
    colliders.push_back(new cyclone::PlaneCollider(cyclone::Vector3(-5, 5, 0), cyclone::Vector3(1, 0, 0))); //left wall
    colliders.push_back(new cyclone::PlaneCollider(cyclone::Vector3(5, 5, 0), cyclone::Vector3(-1, 0, 0))); //right wall
    colliders.push_back(new cyclone::PlaneCollider(cyclone::Vector3(0, 5, -5), cyclone::Vector3(0, 0, 1))); //back wall
    colliders.push_back(new cyclone::PlaneCollider(cyclone::Vector3(0, 5, 5), cyclone::Vector3(0, 0, -1))); //front wall

    constexpr cyclone::real OCTREE_MAX_SIZE = 10;
    constexpr cyclone::real OCTREE_MIN_SIZE = 0.5f;
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyDown(KEY_B)) {
            ...
            newSphere->getRigidbody()->setPosition(cyclone::Vector3(0, 5, 0));
        ...
        std::chrono::milliseconds elapsedShowOctree;
        if (useOctree) {
            ...
            for (int i = 0; i < colliders.size(); i++) {
                if (colliders[i]->getType() == cyclone::ColliderType::Sphere) {
                    std::vector<std::pair<cyclone::OctreeNode<cyclone::Collider*>*, cyclone::Vector3>> nodes = octree.getNodes(colliders[i]->getPosition(), static_cast<const cyclone::SphereCollider &>(*colliders[i]).getRadius() , cyclone::Vector3(0, 5, 0), OCTREE_MAX_SIZE, OCTREE_MIN_SIZE);
        ...
        for(int i = 0; i < colliders.size(); i++) {
            ...
            switch(collider->getType()) {
                ...
                case cyclone::ColliderType::Plane: {
                    ...
                    planeModel.transform = rotation * MatrixTranslate(plane.getPosition().x, plane.getPosition().y, plane.getPosition().z);
        ...
        if (useOctree) {
            ...
            ImGui::Text(("Octree Leaf Size: " + std::to_string(pow(8, OCTREE_LEAF_SIZE)) + "").c_str());
        }
